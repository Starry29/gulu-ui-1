<template>
  ...
  >
      ...
      <transition name="leftArrow">
        <button
          v-show="
            ...
            props.arrow === 'always'
          "
          class="g-carousel-arrow g-carousel-arrow-left"
          @click="prev"
        >
          <svg class="icon">
    ...
    <!--下方定位条-->
    <ul
      ref="carouselIndicatorRef"
      class="g-carousel-indicators"
      :class="indicatorClass"
    >
      <li
        class="g-carousel-indicator"
        v-for="(item, index) in slots"
        @click="handleIndicator(index + 1)"
        @mouseover="hoverIndicator(index + 1)"
      >
...
const props = defineProps({
  ...
  trigger: { type: String, values: ["hover", "click"], default: "click" },
  height: { type: [String, Number] },
});
const indicatorClass = computed(() => {
  return {
    [`g-carousel-indicators-${props.direction}`]: props.direction,
  };
});
...
function useDefaultSlots() {
  slots = ref<Array<CarouselItem>>(
    useSlots()
      .default?.()
      .filter((compo) => compo.type === CarouselItem) ?? []
  );
  if (slots.value.length === 0) {
    useSlots()
      .default?.()
      .forEach((item) => {
        const res = (item["children"] as Array<CarouselItem>).filter(
          (compo) => compo.type === CarouselItem
        );
...
.g-carousel {
  ...
  .g-carousel-container {
    ...
    .g-carousel-arrow {
      ...
      &.g-carousel-arrow-left {
        left: 16px;
      }

      &.g-carousel-arrow-right {
        right: 16px;
      }
  ...
  .g-carousel-indicators {
    ...
    &.g-carousel-indicators-pos-none {
      display: none;
    }

    &.g-carousel-indicators-pos-outside {
      position: relative;
      display: flex;
      justify-content: center;
      align-items: center;

      &.g-carousel-indicators-vertical {
        top: 0;
        left: 2px;
        transform: none;
    ...
    &.g-carousel-indicators-horizontal {
      left: 50%;
      bottom: 0;
